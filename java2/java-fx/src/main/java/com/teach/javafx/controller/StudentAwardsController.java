...
public class StudentAwardsController extends ToolController {
    @FXML
    ...
    @FXML
    private TableColumn<Map,String> awardsNameColumn;//名称
    @FXML
    private TableColumn<Map,String> awardsTypeColumn;
    @FXML
    private TableColumn<Map,String> timeColumn;//
    @FXML
    private TableColumn<Map,String> timeOfAwardsColumn;
    ...
    @FXML
    private TextField awardsNameField;  //学生信息   活动名称
    @FXML
    ...
    public void initialize() {
        ...
        req.add("numName","");
        res = HttpRequestUtil.request("/api/awards/getStudentAwardsList",req); //从后台获取所有学生信息列表集合
    ...
    //更改信息
    protected void changeAwardsInfo() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/awards/getStudentAwardsInfo",req);
        if(res.getCode() != 0){
    ...
    protected void onQueryButtonClick() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/awards/getStudentAwardsList",req);
        if(res != null && res.getCode()== 0) {
            awardsList = (ArrayList<Map>)res.getData();
            setTableViewData();
        }
    ...
    protected void onSaveButtonClick() {
        ...
        req.add("awardsId", awardsId);
        req.add("form", form);
        DataResponse res = HttpRequestUtil.request("/api/awards/awardsStudentEditSave", req);
        if (res.getCode() == 0) {
