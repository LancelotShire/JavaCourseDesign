...
public interface ActivitRepository extends JpaRepository<Activit,Integer> {
      @Query(value="from Activit where (?1=0 or student.studentId=?1)" )
      List<Activit> findByStudentStudentId(Integer studentId);

      Optional<Activit> findByActivitId(Integer activitId);
}
